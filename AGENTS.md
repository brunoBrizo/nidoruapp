...
- Before translating any mobile screen handoff spacing into Expo UI, check the app-wide spacing contract in `apps/mobile/src/global.css` and `apps/mobile/src/design-system/screen.tsx`: use `--spacing-nidoru-screen` via `px-nidoru-screen`/`pl-nidoru-screen`/`pr-nidoru-screen` for standard horizontal screen margins, preserve the shared top/bottom screen rhythm (`pt-12`, `pb-[104px]`) unless the existing screen primitive or handoff explicitly requires an exception, and use `contentInsetAdjustmentBehavior="automatic"` for scrollable tab screens unless the route is a deliberate full-bleed immersive surface.
