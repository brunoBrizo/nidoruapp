...
Every launch loop target must be AAC-LC `.m4a`, at least 240 seconds, loopable without an audible click, and licensed for bundled app distribution before playback tickets can treat it as shippable.

| ID | Display name | Category | Default volume | Target bundled asset | Duration | Loop | License source | Ship status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `light-rain` | Light Rain | Rain | 70% on activation | `apps/mobile/assets/audio/sleep/light-rain.m4a` | Blocked: file missing | Yes | Blocked: no licensed source committed | Blocked |
| `heavy-rain` | Heavy Rain | Rain | 70% on activation | `apps/mobile/assets/audio/sleep/heavy-rain.m4a` | Blocked: file missing | Yes | Blocked: no licensed source committed | Blocked |
| `rain-on-window` | Rain on Window | Rain | 70% on activation | `apps/mobile/assets/audio/sleep/rain-on-window.m4a` | Blocked: file missing | Yes | Blocked: no licensed source committed | Blocked |
| `thunderstorm` | Thunderstorm | Rain | 70% on activation | `apps/mobile/assets/audio/sleep/thunderstorm.m4a` | Blocked: file missing | Yes | Blocked: no licensed source committed | Blocked |
| `ocean-waves` | Ocean Waves | Nature | 70% on activation | `apps/mobile/assets/audio/sleep/ocean-waves.m4a` | Blocked: file missing | Yes | Blocked: no licensed source committed | Blocked |
...
Result on 2026-05-27: directory/file set is absent. Every launch Sound Mixer loop is therefore explicitly blocked until the matching licensed `.m4a` is committed.

Current existing audio inventory:
...
- A real `apps/mobile/assets/audio/sleep/<sound-id>.m4a` file.
