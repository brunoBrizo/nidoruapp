...
- Continuous pink/brown noise is defensible as masking and user preference. Systematic reviews do not support strong sleep-improvement claims. Avoid implying special clinical efficacy for colored noise.
...
| Ambient sound / colored noise | Sleep Sound Mixer includes nature and pink/brown noise | Good for masking and preference; weak evidence for broad sleep claims | Keep as preference tools. Do not sell colored-noise tracks as scientifically proven sleep enhancers |
...
Remaining risky terms in code search are limited to internal identifiers, validation schemas, tests, source quotes, or explicit guardrails. For example, `anxiety_relief` remains a local plan identifier and `daily_practice_hrv` remains internal taxonomy.
